...
function ipad_apps_possible_uses($post){ ?>
  <div id="postdivrich-possible-uses" class="postarea">
    <?php wp_editor(get_post_meta($post->ID, 'ipad_meta_possible_uses', true), 'ipad_meta_possible_uses', array('dfw' => true, 'textarea_rows' => 20, 'tabindex' => 10) ); ?>
  </div>
<?php }
...
function ipad_apps_recommended_by($post) { ?>
  ...
  <input class="p100" name="ipad_meta_recommended_by" tabindex="25" id="ipad_meta_recommended_by" value="<?php echo get_post_meta($post->ID, 'ipad_meta_recommended_by', true) ?>" />
<?php }
